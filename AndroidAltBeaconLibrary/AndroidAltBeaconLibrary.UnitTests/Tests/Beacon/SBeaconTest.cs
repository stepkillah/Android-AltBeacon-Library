...
namespace AndroidAltBeaconLibrary.UnitTests
{
    ...
    public class SBeaconTest : TestBase
    {
        ...
        protected class SBeacon : Beacon
        {
            ...
            private readonly List<Identifier> _newIdentifiers;
            ...
            public SBeacon(int grouping, string id, int time, int txPower, int rssi, int beaconTypeCode, string bluetoothAddress)
            {
                ...
                _newIdentifiers = new List<Identifier>()
                {
                    Identifier.FromInt(grouping),
                    Identifier.Parse(id)
                };
                _time = time;
                HardwareEqualityEnforced = true;
            }
            

            public override int TxPower { get; }

            public override int BeaconTypeCode { get; }

            public override string BluetoothAddress { get; }

            public override IList<Identifier> Identifiers => _newIdentifiers;
        ...
        internal class SBeaconParser : BeaconParser
        {
            ...
            protected override Beacon FromScanData(byte[] bytesToProcess, int rssi, BluetoothDevice device, long timestampMs, Beacon beacon)
            {
                return base.FromScanData(bytesToProcess, rssi, device, timestampMs, beacon);
            }


            public override Beacon FromScanData(byte[] scanData, int rssi, BluetoothDevice device, long timestampMs)
            {
                return this.FromScanData(scanData, rssi, device);
            }
